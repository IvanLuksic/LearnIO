...
module.exports={
    getQuestions: async (req,res,next)=>//ako je dosao do ovde onda je prosao provjere autentikacije
    {
        ...
        const {course_id=1,topic_id=1}=req.body;
        ...
        //1. Vidi jeli se u zadani topic u bazi NIKAD NIJE UŠLO-> boolenablue=FALSE
        try {
            try {
                var blue=await Topic_instance.isBlue(topic_id,course_id,student_id)
            ...
            {
                try {
                    var questions=await Question_instance.getQuestionsFromSave(topic_id,course_id,student_id);
                } catch (error) {
               ...
               response.Questions=questions;
                ...
                try {
                    var questions=await Question_instance.GenerateQuestions(student_id,topic_id,course_id);
                } catch (error) {
            ...
            try {
                matrica=await Topic_instance.GetAsesmentsForTopic(topic_id);
    ...
    checkAnswer: async (req,res,next)=>{
        const {topic_id=1,course_id=1,question_id=20,solution}=req.body;
        const student_id=req.session.user;
        nodelogger.info('Parametri: '+course_id+' '+topic_id+' '+student_id+' '+question_id+'Solution '+solution);
        ...
        try {
             ...
             try {
                 var correct=await Question_instance.checkAnswer(question_id,solution);
             } catch (error) {
             ...
             if(!correct)//odgovor je točan -> 1)Otključaj pitanja 2)Vrati u resposne ponovo sva pitanja sa promijenjenim statusom za ponovno renderiranje matrice
             {
                ...
                try {
                    await Question_instance.UnlockQuestions(student_id,topic_id,course_id,question_id);
                } catch (error) {
